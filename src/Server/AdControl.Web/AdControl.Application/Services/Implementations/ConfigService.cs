...
public class ConfigService : IConfigService
{
    ...
    public async Task<Config> CreateAsync(string name, Guid? userId, IEnumerable<ConfigItem> items, int screensCount, bool isStatic, CancellationToken ct = default)
    {
        var cfg = new Config { Name = name, Id = Guid.NewGuid(), UserId = userId, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow, ScreensCount = screensCount, IsStatic = isStatic};
